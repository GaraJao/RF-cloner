#include <Wire.h>
#include <Adafruit_GFX.h>
#include <Adafruit_SSD1306.h>
#include <RCSwitch.h>

#define OLED_ADDRESS 0x3C // Endereço I2C do display OLED
#define SCREEN_WIDTH 128
#define SCREEN_HEIGHT 64

Adafruit_SSD1306 display(128, 64, &Wire, -1);
RCSwitch mySwitch = RCSwitch();

String received_data = "";
String a_code = "";
String b_code = "";
int flag = 0;
int enterButton = 10, leftButton = 11, rightButton = 12, iPosition = 0, nDigits = 9;
long codeBruteForce;
bool enterPressed;


// 'navegation bar', 128x64px
const unsigned char bitmap_navegation [] PROGMEM = {
  
};


// 'bruteForce', 128x64px
const unsigned char epd_bitmap_bruteForce [] PROGMEM = {
  
};
// 'enviar', 128x64px
const unsigned char epd_bitmap_enviar [] PROGMEM = {
  
};
// 'ouvir', 128x64px
const unsigned char epd_bitmap_ouvir [] PROGMEM = {
  
};


// Array of all bitmaps for convenience. (Total bytes used to store images = 3120)
int lenHome=3;
const unsigned char* bitmap_home[3] = {
    epd_bitmap_ouvir,
    epd_bitmap_enviar,
    epd_bitmap_bruteForce
  
};




void lerDados() {
  while (mySwitch.available()) {
    received_data = mySwitch.getReceivedValue();
    mySwitch.resetAvailable();
    
    switch (flag){
    case 0:
      a_code = received_data;
      flag = 1;
      break;
    default:
      if (a_code == received_data){
        continue; // Ignora o valor repetido e continua para a próxima iteração do loop
      }
      b_code = received_data;
      flag = 0;
      break;
    }
  }

  Serial.print("Código A = ");
  Serial.println(a_code);
  Serial.print("Código B = ");
  Serial.println(b_code);
}


int menuPosition(int len) {
  int n = len-1;
  if(digitalRead(rightButton)==0){
    delay(200);
    iPosition++;
  }else if(digitalRead(leftButton)==0){
    delay(200);
    iPosition--;
  }
  if(iPosition>n){
    iPosition=0;
  }else if(iPosition<0){
    iPosition=n;
  }

}

bool enterStats() {
  if(digitalRead(enterButton) == 0){
  delay(250);
  enterPressed = true;
  return enterPressed;
  }

}

void drawHome(){
  menuPosition(lenHome);
  display.clearDisplay();
  display.drawBitmap(0, 0, bitmap_home[iPosition], 128, 64, SSD1306_WHITE);
  display.display();
  
}

void navegationOuvir(){
  int lenOuvir = 2;
  display.setTextSize(1);
  menuPosition(lenOuvir);
  display.drawBitmap(0, 0, bitmap_navegation, 128, 64, SSD1306_WHITE);
  display.drawRect(5, 16, 118, 36, SSD1306_WHITE);
  display.setTextColor(WHITE);

  display.setCursor(12, 55);
  display.print(" Voltar");
  display.setCursor(74, 55);
  display.print("Resetar");

  display.setTextSize(2);
  display.setCursor(11, 18);
  display.print(a_code);
  
  display.setCursor(11, 35);
  display.print(b_code);

  switch (iPosition){
    case 0: // Voltar
      display.setTextSize(1);
      display.fillRect(5, 53, 59, 16, SSD1306_WHITE);
      display.setTextColor(BLACK);
      display.setCursor(10, 55);
      display.print(" Voltar");
      break;

    default: // Resetar
      display.setTextSize(1);
      display.fillRect(64, 53, 59, 16, SSD1306_WHITE);
      display.setTextColor(BLACK);
      display.setCursor(74, 55);
      display.print("Resetar");
      
      if(enterPressed){ // reseta as variáveis
        a_code = "";
        b_code = "";
        enterPressed = 0;
      }

      break;
  }
}

void navegationEnviar(){
  int lenOuvir = 4;
  display.setTextSize(1);
  menuPosition(lenOuvir);
  display.drawBitmap(0, 0, bitmap_navegation, 128, 64, SSD1306_WHITE);
  
  display.setTextColor(WHITE);
  display.setCursor(12, 55);
  display.print(" Voltar");
  display.setCursor(74, 55);
  display.print("Rolante");

  display.setTextSize(2);
  display.setCursor(12, 18);
  display.print(a_code);
  
  display.setCursor(12, 35);
  display.print(b_code);
  switch (iPosition){
    case 0: // Enviar a_code
      display.setTextSize(2);
      display.fillRect(1, 17, 128, 16, SSD1306_WHITE);
      display.setTextColor(BLACK);
      display.setCursor(12, 18);
      display.print(a_code);
      if(enterPressed){
        enviarCodigos(0);
        delay(1000);
        enterPressed = 0;
      }
      break;

    case 1: // Enviar b_code
      display.setTextSize(2);
      display.fillRect(1, 34, 128, 16, SSD1306_WHITE);
      display.setTextColor(BLACK);
      display.setCursor(12, 35);
      display.print(b_code);
      if(enterPressed){
        enviarCodigos(1);
        delay(1000);
        enterPressed = 0;
      }
      break;

    case 2: // Voltar
      display.setTextSize(1);
      display.fillRect(5, 53, 59, 16, SSD1306_WHITE);
      display.setTextColor(BLACK);
      display.setCursor(10, 55);
      display.print(" Voltar");
      break;

    default: // Enviar A e B
      display.setTextSize(1);
      display.fillRect(64, 53, 59, 16, SSD1306_WHITE);
      display.setTextColor(BLACK);
      display.setCursor(74, 55);
      display.print("Rolante");
      if(enterPressed){
        for (int i = 0; i < 4; i++){
          for (int i2 = 0; i2 < 2; i2++){
            enviarCodigos(i2);
            delay(250);
          }
        }
        
        enterPressed = 0;
      }

      break;
  }
}

void enviarCodigos(int qualEnviar){
  display.clearDisplay();
  display.setTextSize(2);
  display.setTextColor(WHITE);
  display.setCursor(15, 2);
  display.print("Enviando");

  display.setCursor(11, 31);
  switch (qualEnviar) {
    case 0:
      display.print(a_code);
      break;
      
    default:
      display.print(b_code);
      break;
  }
  
  display.display();
  
}

void bruteForce() {
  long minValue = 1; // Determina o valor mínimo para o número de dígitos
  long maxValue = 9; // Determina o valor máximo para o número de dígitos
  int x, textLen;

  // Calcula os valores mínimo e máximo corretamente para o número de dígitos
  for (int i = 1; i < nDigits; i++){
    minValue *= 10;
    maxValue = maxValue * 10 + 9;
  }

  while (minValue <= maxValue && !enterPressed) {
    enterStats();
    display.clearDisplay();
    display.drawBitmap(0, 0, bitmap_navegation, 128, 64, SSD1306_WHITE);
    codeBruteForce = minValue;
    display.setTextColor(WHITE);
    display.setTextSize(2);
    textLen = String(minValue).length(); 
    x = (128 - (12 * textLen))/ 2; // x centralizado
    display.setCursor(2, 24);
    display.print(codeBruteForce);
    display.fillRect(34, 53, 60, 28, SSD1306_WHITE);
    
    display.setTextSize(1);
    display.setTextColor(BLACK);
    display.setCursor(46, 55);
    display.print("Voltar");
    display.display();
    Serial.println(codeBruteForce);
    minValue++;
  }

}



void navegationBruteForce() {
  int lenBruteForce = 3;
  menuPosition(lenBruteForce);
  display.drawBitmap(0, 0, bitmap_navegation, 128, 64, SSD1306_WHITE);
  display.setTextColor(WHITE);
  display.setTextSize(2);
  display.setCursor(21, 24);
  display.print("INICIAR");
  display.setTextSize(1);
  display.setCursor(12, 55);
  display.print(" Voltar");
  display.setCursor(74, 55);
  display.print("Config");

  switch (iPosition){
    case 0: // Iniciar
      display.setTextSize(2);
      display.fillRect(10, 17, 108, 28, SSD1306_WHITE);
      display.setTextColor(BLACK);
      display.setCursor(21, 24);
      display.print("INICIAR");
      if(enterPressed){ // inicia 
        enterPressed = 0;
        while(!enterPressed){
          enterStats();
          display.clearDisplay();
          bruteForce();
          display.display();
        }
       
        enterPressed = 0;
      }
      break;

    case 1: // Voltar
      display.setTextSize(1);
      display.fillRect(5, 53, 59, 16, SSD1306_WHITE);
      display.setTextColor(BLACK);
      display.setCursor(10, 55);
      display.print(" Voltar");

      break;

    default: // Config
      display.setTextSize(1);
      display.fillRect(64, 53, 59, 16, SSD1306_WHITE);
      display.setTextColor(BLACK);
      display.setCursor(74, 55);
      display.print("Config");
      if(enterPressed){ // entra nas configurações
        enterPressed = 0;
        while(!enterPressed){
          enterStats();
          display.clearDisplay(); 
          display.drawBitmap(0, 0, bitmap_navegation, 128, 64, SSD1306_WHITE);
          display.setCursor(21, 24);

          bruteForceDigits();
          display.setTextSize(1);
          display.setCursor(13, 51);
          display.print("NUMERO DE DIGITOS");
          display.display();
        }
        enterPressed = 0;
      }
      break;
    }
}

void bruteForceDigits(){
  int digits = 9;
  display.setTextColor(WHITE);
  display.setTextSize(2);
  display.setCursor(34, 27);
  display.print("<   >");
  menuPosition(digits);
  nDigits = iPosition + 1;
  display.setCursor(58, 27);
  display.print(nDigits);
}



void setup() {
  Serial.begin(9600);
  pinMode(rightButton, INPUT_PULLUP);
  pinMode(leftButton, INPUT_PULLUP);
  pinMode(enterButton, INPUT_PULLUP);
  display.begin(SSD1306_SWITCHCAPVCC, OLED_ADDRESS);
  display.clearDisplay();
  display.setTextColor(WHITE);
  mySwitch.enableReceive(0);  // Receiver on interrupt 0 => that is pin #2
}

void loop() {
  while(!enterPressed){
    enterStats();
    drawHome();
  }
  enterPressed = false;
  switch (iPosition){
  case 0: // Entra em ouvir sinais
    while(!enterPressed){
      display.clearDisplay();
      enterStats();
      navegationOuvir();
      lerDados();
      display.display();
    }
    iPosition = 0;
    enterPressed = false;
    break;

  case 1: // Entra em enviar sinais
    while(!enterPressed){
     display.clearDisplay();
     enterStats();
     navegationEnviar();
     display.display();
    }
    iPosition = 0;
    enterPressed = false;
    break;
  
   default: // Entra em força bruta
    display.clearDisplay();
    iPosition = 0;

    while(!enterPressed){
      display.clearDisplay();
      enterStats();
      navegationBruteForce();
      display.display();
    }
    iPosition = 0;
    enterPressed = false;
    break;
  }
 
  display.clearDisplay();
 
}
/*
 * teste 1 Código A = 143799829
           Código B = 119195496

   teste 2 Código A = 28238280 
           Código B = 143799829

   teste 3 Código A = 224714664
           Código B = 143799845

   teste 4 Código A = 143799845
           Código B = 66417304

   teste 5 Código A = 143799845
           Código B = 255806712




    




 */
